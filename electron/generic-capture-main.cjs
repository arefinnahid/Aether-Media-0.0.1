...
module.exports = function createGenericCaptureHelpers(deps) {
  ...
  const RE_BLANK_MP4 = /(?:blank|black[_\-]screen|placeholder|dummy)[\w\-\.]*\.(?:mp4|webm)/i;
  ...
  const RE_PREVIEW_FILENAME = /(?:^|[\/_\-.])(?:thumb|preview|poster|small|mini|tiny|gif|sprite|placeholder|loading|sample_thumb|hover|black[_\-]screen|blank|dummy)[_\-.\d]*\.(?:mp4|webm|mkv)/i;
  ...
  const isLikelyPageUrl = (url) => {
    ...
    try {
      ...
      // NEW: Root paths are almost always pages (e.g. https://player.example.com/?video=1)
      if (path === '/' || path === '') {
        const hasMediaParam = ['file', 'source', 'src', 'stream', 'video', 'm3u8'].some(p => u.searchParams.has(p));
        if (!hasMediaParam) return true;
      }
  ...
  const scoreUrl = (url, context = {}) => {
    ...
    if (isLikelyPreviewMedia(url)) score -= 5000;
    if (/(?:preview|sample|trailer|teaser|thumb|black[_\-]screen|dummy|blank)/i.test(lower)) score -= 5000;
    ...
    // Penalize URLs found purely via regex/text scanning that haven't been confirmed by any network or DOM element activity
    const isUnverifiedScan = !context.fromVideoElement &&
      !context.fromMediaResource &&
      !context.confirmedByContentType &&
      !context.fromStructuredData &&
      !context.fromApiJson &&
      (context.contentLength === undefined || context.contentLength === 0) &&
      !context.fromDownloadLink &&
      !context.fromPreloadLink;
    if (isUnverifiedScan) {
      score -= 300;
    }
    ...
    if (RE_M3U8.test(lower) && !RE_MASTER_M3U8.test(lower) && !RE_INDEX_M3U8.test(lower) && !/\/(?:hls|m3u8s?|dash)\//i.test(lower)) {
      // Could be a variant playlist – still useful but less than master
      score -= 20;
    }

    // ── 6. Manifest path bonus ──
    // URLs containing /hls/, /m3u8s?/, or /dash/ in path are strong signals
    if (/\/(?:hls|m3u8s?)\//i.test(lower)) score += 40;
  ...
  async function fetchMainPlayableVideoUrl(pageUrl, onProgress, depth = 0, customReferer = null) {
    ...
    const setUrlContext = (url, ctx) => {
      const resolved = resolveCapturedUrlAgainstPage(url, pageUrl) || url;
      const existing = urlContext.get(resolved) || {};
      urlContext.set(resolved, { ...existing, ...ctx });
    };
    ...
    /** Deduplicating add – returns true if the URL was actually new */
    ...
    const onMediaCaptured = (url, context = {}) => {
      ...
      // Don't trigger settle on very low-score captures (thumbnails, previews, etc.)
          ...
          if ((ct.includes('json') || ct.includes('ld+json')) && (rt === 'fetch' || rt === 'xhr')) {
            ...
            if (matches) {
              for (const raw of matches) {
                ...
                if (cleaned && shouldAcceptCapturedUrl(cleaned, 'json-mine')) {
                  const ctx = { fromApiJson: true, source: 'json-mine' };
                  setUrlContext(cleaned, ctx);
                  if (addCapturedUrl(cleaned)) {
                    console.log(`[PLAYWRIGHT][JSON-MINE] ${cleaned}`);
                    onMediaCaptured(cleaned, urlContext.get(cleaned));
                  }
                }
              }
            }
              ...
              const scanJson = (obj, depth = 0) => {
                ...
                if (obj.dash && typeof obj.dash === 'object') {
                  ...
                  for (const su of streamUrls) {
                    const ctx = { source: 'json-dash-scan' };
                    setUrlContext(su, ctx);
                    if (addCapturedUrl(su)) {
                      console.log(`[PLAYWRIGHT][JSON-DASH] ${su}`);
                      onMediaCaptured(su, urlContext.get(su));
                    }
                  }
                }
            ...
            if (relMatches) {
              for (const raw of relMatches) {
                try {
                  ...
                  if (shouldAcceptCapturedUrl(resolved, 'json-relative-mine')) {
                    const ctx = { fromApiJson: true, source: 'json-relative-mine' };
                    setUrlContext(resolved, ctx);
                    if (addCapturedUrl(resolved)) {
                      console.log(`[PLAYWRIGHT][JSON-REL-MINE] ${resolved}`);
                      onMediaCaptured(resolved, urlContext.get(resolved));
                    }
                  }
                } catch { }
              }
            ...
            if (cdnMatches) {
              for (const raw of cdnMatches) {
                const cleaned = cleanExtractedUrl(raw);
                if (cleaned && shouldAcceptCapturedUrl(cleaned, 'json-cdn-mine')) {
                  const ctx = { fromCdnJsonMine: true, source: 'json-cdn-mine' };
                  setUrlContext(cleaned, ctx);
                  if (addCapturedUrl(cleaned)) {
            ...
            try {
              ...
              const mineObject = (obj, depth = 0) => {
                ...
                for (const [rawKey, rawVal] of Object.entries(obj)) {
                  ...
                  if ((isMediaUrl(rawVal) || isCdnHostname(extractHostname(rawVal)))) {
                    const sfxCtx = { fromApiJson: true };
                    setUrlContext(rawVal, sfxCtx);
                    if (addCapturedUrl(rawVal)) {
                      console.log(`[PLAYWRIGHT][JSON-SUFFIX-KEY:${key}] ${rawVal}`);
                      onMediaCaptured(rawVal, urlContext.get(rawVal));
                    }
                  }
                }
        ...
        if (structuredMedia && structuredMedia.length > 0) {
          ...
          for (const u of structuredMedia) {
            if (addCapturedUrl(u)) {
              ...
              setUrlContext(u, { source: 'universal-state-scan' });
      ...
      const processDomPollResults = (videoSrcs, discoveredUrls, subs, label = '') => {
        ...
        for (const url of videoSrcs) {
          ...
          if (shouldAcceptCapturedUrl(url, 'video-src') && addCapturedUrl(url)) {
            ...
            // Only trigger settle for non-preview video sources
            if (!isPreview) {
